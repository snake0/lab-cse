...
public:
    ...
    explicit HdfsException(const char *what) : runtime_error(what) {}
};
...
class NameNode {
    struct LocatedBlock {
        ...
        LocatedBlock(blockid_t block_id, uint64_t offset, uint64_t size, const DatanodeIDProto &loc) :
                block_id(block_id), offset(offset), size(size), locs(1, loc) {}
